...
type L2ContractEvent struct {
	...
	BlockHash       string `gorm:"type:varchar(256);not null;index:idx_l2_contract_events_block_hash_log_index_key,priority:1"`
	ContractAddress string `gorm:"type:varchar(256);not nul"`
	TransactionHash string `gorm:"type:varchar(256);not null"`
	LogIndex        int    `gorm:"type:integer;not null;index:idx_l2_contract_events_block_hash_log_index_key,priority:2"`
